...
- A zone's `xf` is attached **only when non-identity**; `ALL[cont].links` is emitted **only when non-empty**. **Both are now populated in practice** — Antonica ships 46 non-identity `zoneXf` entries and 11 `links`, Kunark 26 and 19. (This invalidates an earlier note here claiming every continent ships identity `zoneXf` and empty `links`. It matters: any code that treats "identity" as a proxy for "the author didn't set this" is wrong — see the sparseness rule in `customization-overlay.md`.)
