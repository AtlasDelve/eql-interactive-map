...
| `src/mapgeom.js`; browser-side geometry, walk-cost arithmetic, zone-name resolution | [`travel-graph.md`](docs/reference/travel-graph.md) |
...
| `src/pack_convert.js` — the DOM-free browser twin of pack import, geometry/cost, and build composition | [pack-import.md](docs/reference/pack-import.md) + [travel-graph.md](docs/reference/travel-graph.md) + Build pipeline below |
...
- **The zone-name resolution tables — `znorm`/`ZIDX`/`ZALIAS`/`LINK_OVERRIDE`, which turn a label into a zone key — exist in three named copies:** `scripts/mapgeom.py`, `src/template.html`, and `src/mapgeom.js`. `travel-full.test.js` guards the template/Python edge by recomputing every authored cost through the template; `mapgeom.test.js` differentially guards Python/JavaScript parity, while its absolute transform round-trip catches a bug ported faithfully into both. Change one copy, change all three (`travel-graph.md`).
